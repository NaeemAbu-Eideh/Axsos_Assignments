...
<html>
    ...
    <body class="overflow-y-hidden">
        <h1 class="text-3xl font-bold mt-10 mb-15 text-center "> <c:out value="${category.name}"/> </h1>
        ...
        <div class="w-400 mx-auto mb-10 pb-5 border-b-2">
            ...
            <ul class="list-disc list-decimal mt-7 ml-10">
                <c:forEach var="product" items="${category.products}">
                    <li class="ml-6"> <c:out value="${product.name}"/> </li>
                </c:forEach>
            </ul>
        </div>

        <form action="/categories/${category.id}/add" method="post">
            <select class="border w-400 block mx-auto" name="product_id">
